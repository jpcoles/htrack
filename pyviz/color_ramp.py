def color_ramp_grey(r, g, b):
    c = sqrtf(r**2 + g**2 + b**2)
    if c > 1.0: c = 1.0
    return c,c,c

def color_ramp_hot2cold(r, g, b):
    cr = cg = cb = 1.0

    v = sqrt(r**2 + g**2 + b**2)

    if (v < 0.0):
      v = 0
    elif (v > 1.0):
      v = 1.0

    if v < 0.25:
      cr = 0
      cg = 4 * v
    elif v < (0.5):
      cr = 0
      cb = 1 + 4 * (0.25 - v)
    elif v < (0.75):
      cr = 4 * (v - 0.5)
      cb = 0
    else:
      cg = 1 + 4 * (0.75 - v)
      cb = 0

    return cr,cg,cb

astro_ramp = None
def color_ramp_astro(r, g, b):

    global astro_ramp
    if not astro_ramp:
        astro_ramp = [
          [0,0,255], [0,0,254], [1,0,253], [2,0,252], [3,0,250], [4,0,249], [5,0,248], [6,0,247], [7,0,247],
          [8,0,246], [9,0,245], [10,0,244], [11,0,242], [12,0,241], [13,0,240], [14,0,239], [15,0,239], [16,0,238],
         [17,0,237], [18,0,236], [19,0,234], [20,0,233], [21,0,232], [22,0,231], [23,0,231], [24,0,230], [25,0,229],
         [26,0,228], [27,0,226], [28,0,225], [29,0,224], [30,0,223], [31,0,223], [33,0,222], [33,0,221], [35,0,220],
         [35,0,218], [37,0,217], [37,0,216], [39,0,215], [39,0,215], [41,0,214], [41,0,213], [43,0,212], [43,0,210],
         [45,0,209], [45,0,208], [47,0,207], [47,0,207], [49,0,206], [49,0,205], [51,0,204], [51,0,202], [53,0,201],
         [53,0,200], [55,0,199], [55,0,199], [57,0,198], [57,0,197], [59,0,196], [59,0,194], [61,0,193], [61,0,192],
         [63,0,191], [63,0,191], [64,0,190], [66,0,189], [67,0,188], [67,0,186], [68,0,185], [70,0,184], [71,0,183],
         [71,0,183], [72,0,182], [74,0,181], [75,0,180], [75,0,178], [76,0,177], [78,0,176], [79,0,175], [79,0,175],
         [80,0,174], [82,0,173], [83,0,172], [83,0,170], [84,0,169], [86,0,168], [87,0,167], [87,0,167], [88,0,166],
         [90,0,165], [91,0,164], [91,0,162], [92,0,161], [94,0,160], [95,0,159], [95,0,159], [96,0,158], [98,0,157],
         [99,0,156], [99,0,154], [100,0,153], [102,0,152], [103,0,151], [103,0,151], [104,0,150], [106,0,149], [107,0,148],
        [107,0,146], [108,0,145], [110,0,144], [111,0,143], [111,0,143], [112,0,142], [114,0,141], [115,0,140], [115,0,138],
        [116,0,137], [118,0,136], [119,0,135], [119,0,135], [120,0,134], [122,0,133], [123,0,132], [123,0,130], [124,0,129],
        [126,0,128], [127,0,127], [127,0,127], [128,0,126], [129,0,124], [130,0,123], [132,0,123], [133,0,122], [134,0,120],
        [135,0,119], [135,0,119], [136,0,118], [137,0,116], [138,0,115], [140,0,115], [141,0,114], [142,0,112], [143,0,111],
        [143,0,111], [144,0,110], [145,0,108], [146,0,107], [148,0,107], [149,0,106], [150,0,104], [151,0,103], [151,0,103],
        [152,0,102], [153,0,100], [154,0, 99], [156,0, 99], [157,0, 98], [158,0, 96], [159,0, 95], [159,0, 95], [160,0, 94],
        [161,0, 92], [162,0, 91], [164,0, 91], [165,0, 90], [166,0, 88], [167,0, 87], [167,0, 87], [168,0, 86], [169,0, 84],
        [170,0, 83], [172,0, 83], [173,0, 82], [174,0, 80], [175,0, 79], [175,0, 79], [176,0, 78], [177,0, 76], [178,0, 75],
        [180,0, 75], [181,0, 74], [182,0, 72], [183,0, 71], [183,0, 71], [184,0, 70], [185,0, 68], [186,0, 67], [188,0, 67],
        [189,0, 66], [190,0, 64], [191,0, 63], [191,0, 63], [192,0, 61], [193,0, 61], [194,0, 59], [196,0, 59], [197,0, 57],
        [198,0, 57], [199,0, 55], [199,0, 55], [200,0, 53], [201,0, 53], [202,0, 51], [204,0, 51], [205,0, 49], [206,0, 49],
        [207,0, 47], [207,0, 47], [208,0, 45], [209,0, 45], [210,0, 43], [212,0, 43], [213,0, 41], [214,0, 41], [215,0, 39],
        [215,0, 39], [216,0, 37], [217,0, 37], [218,0, 35], [220,0, 35], [221,0, 33], [222,0, 33], [223,0, 31], [223,0, 30],
        [224,0, 29], [225,0, 28], [226,0, 27], [228,0, 26], [229,0, 25], [230,0, 24], [231,0, 23], [231,0, 22], [232,0, 21],
        [233,0, 20], [234,0, 19], [236,0, 18], [237,0, 17], [238,0, 16], [239,0, 15], [239,0, 14], [240,0, 13], [241,0, 12],
        [242,0, 11], [244,0, 10], [245,0,  9], [246,0,  8], [247,0,  7], [247,0,  6], [248,0,  5], [249,0,  4], [250,0,  3],
        [252,0,  2], [253,0,  1], [254,0,  0], [255,0,  0]]

    v = sqrt(r**2 + g**2 + b**2)
    i = v * 256
    if i > 255: i = 255
    return astro_ramp[i][0] / 255.0, astro_ramp[i][1] / 255.0, astro_ramp[i][2] / 255.0

tipsy_ramp = None
def color_ramp_tipsy(f):

    global tipsy_ramp

    if not tipsy_ramp:
        tipsy_ramp =  [
            [255, 255, 255], [0, 255, 0], [190, 200, 255], [255, 63, 63], [255, 255, 140], [127, 255, 212], [255, 160, 122],
            [238, 224, 229], [255, 239, 213], [238, 233, 191], [72,  118, 255], [192, 192, 192], [255, 231, 186],
            [0,   154, 205], [102, 205, 0], [240, 255, 240], [180, 205, 205], [255, 245, 238],
            [205, 133, 63], [108, 133, 139], [171, 130, 255], [255, 228, 196], [238, 92, 56],
            [0,205, 205], [178, 34, 34], [139, 95, 101], [255, 228, 225], [238, 232, 170],
            [179, 238, 58], [121, 160, 221], [255, 0,0], [205, 198,115], [255, 215,185],
            [238, 54,0], [155, 205,155], [173, 255,47], [238, 122,233], [0,0,128],
            [222,184,135], [205,41,144], [245,255,250], [138,43,226], [125,38,205],
            [0,139,69], [34,139,34], [238,58,140], [47,79,79], [188,143,143],
            [124,252,0], [255,228,181], [239,237,161], [65,105,225], [255, 238,104],
            [238,106,80], [0,100,0], [204,190,112], [205,200,177], [0,255,127],
            [240,255,255], [160,82,45], [46,139,87], [0,197,205], [205,102,0],
            [205,96,144], [122,197,205], [176,48,96], [209,95,238], [255,240,245],
            [205,102,29], [79,148,205], [255,255,240], [208,32,144], [0,206,209],
            [141,238,238], [205,201,201], [143,188, 143], [240,248,255], [255,255,0],
            [135,206,235], [220,220,220], [238,99,99], [100,149,237], [255,235,205],
            [238,153,73], [238,18,137], [25,25,112], [205,112,84], [60,179,113],
            [255,250,205], [139,26,26], [173,216,230], [211,211,211], [205,192,176],
            [238,213,210], [110,139,61], [139,134, 130], [102,205,170], [205,205,180],
            [205,145,115], [0,229,238], [205,104,57], [32,178,170], [137,104,205],
            [139,69,19], [132,112,255], [255,246,143], [216,191,216], [154,50,205],
            [253,245,230], [239,87,66], [205,104,137], [102,205,170], [205,133,0],
            [239,62,47], [105,105,105], [139,125,107], [0,0,205], [105,139,34],
            [184,134,11], [70,130,180], [84,139,84], [255,48,48], [238,174,238],
            [112,128,144], [255,69,0], [154,192,205]]

        for i in range(128+2):
            tipsy_ramp.append(tipsy_ramp[i])

    i = int(f * 255)
    if i > 255: i = 255
    return tipsy_ramp[i][0] / 255.0, tipsy_ramp[i][1] / 255.0, tipsy_ramp[i][2] / 255.0

wrbb_ramp = None
def color_ramp_wrbb(r, g, b):

    global wrbb_ramp
    if not wrbb_ramp:
        slope = 255./20.
        for i in range(21):
            wrbb_ramp.append([0,0,(int)(slope * (double)(i) + .5)])

        slope = 191./20.
        offset = 64. - slope * 21.
        for i in range(21,42):
            wrbb_ramp.append([(int)(slope * (double)(i) + .5), 0, 255])

        slope = -205./21.
        offset = 255. - slope * 41.
        for i in range(22,63):
            wrbb_ramp.append([255,0,(int)(slope * (double)(i) + .5)])

        slope = 205./40.
        offset = 50. - slope * 63.
        for i in range(63,104):
            wrbb_ramp.append([255,(int)(slope * (double)(i) + .5), 0])

        slope = 255./21.
        offset = -slope * 103.
        for i in range(104,125):
            wrbb_ramp.append([255,255,(int)(slope * (double)(i) + .5)])

        for i in range(125):
            wrbb_ramp.append(wrbb_ramp[i])

    v = sqrt(r**2 + g**2 + b**2)
    i = int(v * 256)
    if i > 255: i = 255

    return wrbb_ramp[i][0] / 255.0, wrbb_ramp[i][1] / 255.0, wrbb_ramp[i][2] / 255.0

